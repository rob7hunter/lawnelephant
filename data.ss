...
(define completed? (cut has-tag? <> "complete"))

;; note that we don't implement this as
;; (rec-prop fr-rec 'body "missing") because we want to catch, in
...
(define (post-body rec)
  (or (aand (rec-prop rec 'body) (any-body-markup it))
      "missing"))

(define (post-body-no-markup rec)
  (or (rec-prop rec 'body) "missing"))

(define (any-body-markup str)
  (markup-body (string-trim str)))

(define (feature-request-validator fr-rec)
  (let ((lookup (rec-prop fr-rec 'body)))
